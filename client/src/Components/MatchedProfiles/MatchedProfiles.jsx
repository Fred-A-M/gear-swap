  ...
  return (
    ...
    {matchedProfiles.length === 0 ? (
      <div className='w-full flex justify-center'>
        <div className='font-bold text-xl text-white text-center justify-center p-3 bg-orange-400 rounded-2xl w-auto mt-6 mb-4'>
          ...
          <>
          <div className='w-full flex justify-center'>
            <div className='font-bold text-white text-center justify-center p-1 bg-orange-400 rounded-2xl w-52 mt-6 mb-4'>
              Matched Users
            </div>
          </div>
          <div className="w-max mx-auto">
            <div className={`matched-profiles-container grid grid-cols-1 sm:grid-cols-${matchedProfiles.length >= 2 ? 2 : 1} md:grid-cols-${matchedProfiles.length >= 3 ? 3 : matchedProfiles.length} lg:grid-cols-${matchedProfiles.length >= 4 ? 4 : matchedProfiles.length} justify-center gap-8 pb-11`}>
                {matchedProfiles.map(function(profile) {
                  return (
                    <div key={profile._id} className='hover:cursor-pointer transition duration-300 hover:drop-shadow-2xl bg-orange-100 hover:bg-orange-200 h-52 w-52  rounded-2xl flex flex-col items-center drop-shadow-xl' onClick={() => {handleProfileClick(profile._id); changeState('user')}}>
                      <div className='font-bold mt-1'>
                          {profile.username}
                      </div>
                      <div className='profile-gear-list self-center mt-3 mb-2'>
